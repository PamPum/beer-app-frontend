...
import constants from "./constants/beertionary-admin.handlers.constants";
// Libs
...
const submitHandlerEdit = async ({
  ...
  try {
    ...
    setOpenDialog(false);
    setData((prev) =>
      prev.map((item) => (item._id === selectedRow._id ? data.entry : item))
    );
    toast.success(data.message);
  } catch (error) {
    if (error.response.data.errors) {
...
const BeertionaryAdminHandlers = ({
  form,
  openAlert,
  openDialog,
  selectedRow,
...
export default BeertionaryAdminHandlers;
